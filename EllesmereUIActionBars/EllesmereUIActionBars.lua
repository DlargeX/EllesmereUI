...
local function SetupExtraBarHolder(barKey, frameName, barInfo)
    ...
    -- and re-acquire via UpdatePosition hook only (Bartender4/Dominos pattern).
    if barInfo and barInfo.blizzOwnedVisibility then
        ...
        -- Reclaim via UpdatePosition hook only (Bartender4/Dominos pattern).
    ...
    -- menu via the higher-level ActionBarController_UpdateAll hook (Bartender4
    -- pattern) and event listeners.
    ...
    if barKey == "MicroBar" then
        -- Just anchor the holder to track MicroMenuContainer's position
        -- without reparenting. The holder exists for unlock mode only.
        local function SyncHolderToBlizz()
            local fw, fh = blizzFrame:GetWidth(), blizzFrame:GetHeight()
            if fw and fw > 1 and fh and fh > 1 then
                holder:SetSize(fw, fh)
            end
        ...
        SyncHolderToBlizz()
        blizzFrame:HookScript("OnSizeChanged", SyncHolderToBlizz)
        return holder
    end
